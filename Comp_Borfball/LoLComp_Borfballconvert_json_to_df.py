...
def convert_json_to_df(trials_json):
    ...
    for key in trials_json['compcheck'].keys():
        df[key] = trials_json['compcheck'][key]

    for key in trials_json['resemblance'].keys():
        df[key] = trials_json['resemblance'][key]     
...
with open('LoLCompResults_SAVE_SONA.json') as json_file:
...
df_trials.to_csv("LoLCompResults_SONA.csv")
